...
@hsu2006cap argues that VW portfolios are sub-optimal because they mechanically overweight overpriced stocks and underweight underpriced stocks—any deviation of price from fundamental value creates a systematic drag on VW performance relative to a fundamentally weighted alternative.
...
@plyakha2021equal decompose the EW outperformance over VW into three components:
...
Between VW and EW lies a continuum of weighting schemes. @tbl-schemes summarizes the major alternatives.
...
: Summary of portfolio weighting schemes. {#tbl-schemes}
...
```{python}
#| label: data-load
#| code-summary: "Load market and fundamental data from DataCore.vn"
...
def construct_universe(monthly_df, min_mcap_pct=0, min_turnover=0,
                       min_months=12):
    """
...
universe_mid = construct_universe(monthly, min_mcap_pct=20,
                                  min_turnover=0.5)
universe_liquid = construct_universe(monthly, min_mcap_pct=40,
                                     min_turnover=2.0)

for name, univ in [('All stocks', universe_all),
...
def compute_portfolio_returns(monthly_df, weight_fn, rebal_freq='M',
    ...
    if rebal_freq == 'M':
        ...
        rebal_dates = set(
            pd.to_datetime(months).to_period('Q').to_timestamp('M')
        )
    ...
    for month in months:
        ...
        if prev_weights is not None:
            ...
            prev_drifted = prev_aligned * (1 + cross_section['monthly_return'].reindex(prev_aligned.index, fill_value=0))
            prev_drifted = prev_drifted / prev_drifted.sum() if prev_drifted.sum() > 0 else prev_drifted
...
def fw_weights(cross_section):
    ...
    for col in ['revenue', 'book_equity', 'dividends_paid',
                'operating_cash_flow']:
...
fw_returns = compute_portfolio_returns(
    monthly_fund, fw_weights, rebal_freq='A'
)
print(f"Fundamental-weighted: mean ret = {fw_returns['port_return'].mean():.4f}, "
      f"eff_N = {fw_returns['effective_n'].mean():.1f}")
```

## Risk-Based Weighting Schemes {#sec-weight-risk-based}

The weighting schemes above use only market cap or accounting data. Risk-based schemes incorporate the covariance structure of returns, aiming to produce portfolios with better risk-adjusted performance. The trade-off is that they require estimating the covariance matrix—a high-dimensional object that is notoriously difficult to estimate precisely with short time series.
...
The global minimum variance (GMV) portfolio minimizes portfolio variance without targeting a specific return level [@clarke2006minimum]:
...
@choueifaty2008toward define the diversification ratio as the ratio of weighted average volatility to portfolio volatility:
...
: Explicit transaction cost components for Vietnamese equities. {#tbl-costs}
...
: Summary comparison of weighting schemes for Vietnamese equities. {#tbl-final-comparison}

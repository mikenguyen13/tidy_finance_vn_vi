# Phân loại danh mục đầu tư đơn biến

Trong chương này, chúng ta sẽ đi sâu vào phân loại danh mục đầu tư, một trong những phương pháp thống kê được sử dụng rộng rãi nhất trong định giá tài sản thực nghiệm [ví dụ: @BaliEngleMurray2016]. Ứng dụng chính của phân loại danh mục đầu tư là để kiểm tra xem một hoặc nhiều biến số có thể dự đoán lợi nhuận vượt trội trong tương lai hay không. Nói chung, ý tưởng là phân loại các cổ phiếu riêng lẻ vào các danh mục đầu tư, trong đó các cổ phiếu trong mỗi danh mục đầu tư tương tự nhau về một biến số phân loại, chẳng hạn như quy mô công ty. Các danh mục đầu tư khác nhau sau đó đại diện cho các khoản đầu tư đa dạng tốt, khác nhau về mức độ của biến số phân loại. Sau đó, bạn có thể quy kết sự khác biệt trong phân phối lợi nhuận cho tác động của biến số phân loại. Chúng ta bắt đầu bằng cách giới thiệu phân loại danh mục đầu tư đơn biến (phân loại chỉ dựa trên một đặc điểm) và giải quyết phân loại hai biến trong phần Giá trị và Phân loại Hai biến.

Phân loại danh mục đầu tư đơn biến chỉ xem xét một biến phân loại duy nhất là $x_{i,t-1}$. Ở đây, $i$ biểu thị cổ phiếu và $t-1$ cho biết đặc điểm này có thể quan sát được bởi các nhà đầu tư tại thời điểm $t$. Mục tiêu là đánh giá mối quan hệ chéo giữa $x_{i,t-1}$ và, thông thường, lợi nhuận vượt trội của cổ phiếu $r_{i,t}$ tại thời điểm $t$ như là biến kết quả. Để minh họa cách thức hoạt động của phân loại danh mục đầu tư, chúng ta sử dụng các ước tính về hệ số beta thị trường từ chương trước làm biến phân loại của mình.
...
Chúng ta bắt đầu bằng việc tải dữ liệu cần thiết từ cơ sở dữ liệu SQLite đã được giới thiệu trong phần Truy cập và Quản lý Dữ liệu Tài chính và Dữ liệu DataCore. Cụ thể, chúng ta sử dụng dữ liệu giá cổ phiếu hàng tháng làm tập hợp tài sản. Sau khi hình thành các danh mục đầu tư, chúng ta sử dụng lợi nhuận của các yếu tố thị trường để tính toán hiệu suất điều chỉnh rủi ro (tức là alpha). `beta` là khung dữ liệu chứa các hệ số beta thị trường được tính toán trong chương trước.

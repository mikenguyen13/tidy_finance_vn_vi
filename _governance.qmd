...
-   `symbol`: The stock symbol symbol on HOSE or HNX.
...
We compute the governance index by summing all binary governance provision indicators. The provision columns are identified by the prefix `gov_`.
...
We merge the governance classifications with monthly stock return data from DataCore.vn.
...
Raw returns alone do not tell us whether the governance strategy generates **abnormal** returns---returns that cannot be explained by exposure to common risk factors. We estimate the following four-factor model:
...
factors_monthly = pd.read_csv(
    "data/datacore_factors_monthly.csv",
    parse_dates=["date"]
)

factors_monthly.info()
```
...
factors_monthly["ym"] = factors_monthly["date"].dt.to_period("M")

analysis_data = returns_wide.merge(
    factors_monthly[["ym", "mktrf", "smb", "hml", "umd", "rf"]],
    on="ym",
    how="inner"
)
...
-   @Bebchuk2009 refined the G-Index to a more parsimonious **Entrenchment Index (E-Index)** based on only six provisions that matter most for firm value: staggered boards, limits to shareholder bylaw amendments, poison pills, golden parachutes, and supermajority requirements for mergers and charter amendments. The E-Index explained the governance-return relationship at least as well as the full G-Index.

-   @Cremers2005 found that the governance effect was strongest in firms where external governance mechanisms (the takeover market) were active, suggesting complementarity between internal and external governance.

-   The governance premium in the US has largely disappeared in more recent periods [@Bebchuk2013], potentially because the market has learned to price governance differences. This "learning hypothesis" has implications for whether similar strategies can persist in less efficient markets like Vietnam.
...
1.  **Information asymmetry**: Weaker disclosure requirements and less analyst coverage mean governance quality is harder to observe, creating larger mispricings [@Klapper2004].

2.  **Weaker legal enforcement**: Where legal protections for minority shareholders are weak, firm-level governance becomes more important as a substitute [@LaPorta2000].

3.  **Concentrated ownership**: The presence of controlling shareholders creates opportunities for tunneling and related-party transactions that good governance mechanisms can mitigate [@Johnson2000].
...
4.  **Index construction**: The specific provisions included in the VN-GIndex and their equal weighting may not optimally capture governance quality. Future work could explore weighted indices, following @Bebchuk2009.

## Exercises

1.  **Entrenchment Index**: Following @Bebchuk2009, identify the subset of governance provisions in the Vietnamese data that have the strongest association with firm value (measured by Tobin's Q or market-to-book ratio). Construct a Vietnamese E-Index using only these provisions and compare its predictive power for returns to the full VN-GIndex.
...
The analysis provides a template for evaluating whether corporate governance is priced in Vietnam's equity market---a question with important implications for both investors and policymakers seeking to strengthen the Vietnamese capital market's governance infrastructure.
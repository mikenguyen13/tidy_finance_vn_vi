...
[^1]: Vietnamese text requires specialized tokenization due to compound words (e.g., "công ty" = company, "thị trường" = market).
...
[^2]: @loughran2011liability show that general-purpose dictionaries misclassify up to 73% of negative words in financial text.

@hoberg2010product and @hoberg2016text pioneered the use of product descriptions from 10-K filings to construct text-based industry classifications (TNIC), demonstrating that these dynamic, firm-specific industry definitions outperform static SIC and NAICS codes in explaining firm behavior including profitability, stock returns, and M&A activity. Subsequent work by @hoberg2018text extended this to measure competitive threats and product market fluidity.
...
We construct the universe of Vietnamese listed firms using DataCore.vn's comprehensive database. The universe includes all firms listed on HOSE, HNX, and UPCoM as of the analysis date. DataCore.vn provides OHLCV price data, financial statements, ownership data, corporate events, and crucially for this chapter, business descriptions and annual report text in Vietnamese.
...
::: callout-note
Market capitalization figures are illustrative. Actual values should be retrieved from DataCore.vn at the time of analysis.
:::

## Retrieving Business Descriptions {#sec-bus-desc}

DataCore.vn stores business descriptions for all listed firms in both Vietnamese and English (where available). We retrieve both versions for our analysis. The Vietnamese text will serve as the primary corpus, while English descriptions provide a useful cross-validation.

```{python}
#| label: get-bus-desc
#| code-summary: "Retrieve business descriptions from DataCore.vn"
...
Beyond business descriptions, annual reports provide richer and more time-varying textual data. DataCore.vn archives parsed annual report text for all listed firms. We extract the Management Discussion and Analysis (MD&A) sections, which are most informative for financial analysis [@li2010information; @bonsall2017plain]. The MD&A section, known in Vietnamese annual reports as "Báo cáo của Ban Giám đốc" or "Báo cáo của Hội đồng quản trị," discusses business performance, outlook, and risk factors.
...
Latent Dirichlet Allocation [@blei2003latent] is a generative probabilistic model that discovers latent topics in a corpus. Each document is modeled as a mixture of topics, and each topic is a distribution over words. LDA has been widely applied in finance to identify thematic content in 10-K filings [@dyer2017evolution], earnings calls [@huang2018analyst], and news articles [@bybee2023business].
...
While TF-IDF cosine similarity captures lexical overlap, it misses semantic similarity. Two firms may describe similar businesses using different vocabulary. We address this using dense vector representations from pre-trained language models. Specifically, we compute document embeddings using Sentence-BERT [@reimers2019sentence] with a Vietnamese bi-encoder model.[^3]

[^3]: @reimers2019sentence demonstrate that sentence-BERT embeddings reduce computation for similarity tasks from 65 hours to 5 seconds on 10,000 sentence pairs.
